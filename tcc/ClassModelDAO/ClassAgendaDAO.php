                ...
                if($agenda->getIntervalo() == 1){
                    ...
                    if($agenda->getIntervalo() == 2){
                        ...
                        do{
                            ...
                            $tempDataAgenda = date('Y-m-d H:i', strtotime("+{$agenda->getDiferenca()} days", strtotime($tempDataAgenda)));
                            $dataAgenda = date('Y-m-d', strtotime($tempDataAgenda));
                            $horario = date('H:i', strtotime($tempDataAgenda));

                            $i++;
                        }while($i < $agenda->getqtdeDias());
                    }else if($agenda->getIntervalo() == 4){
                        $qtdeDoses = ((24 / $agenda->getDiferenca()) * $agenda->getqtdeDias());
                        ...
                        do{
                            ...
                            $tempDataAgenda = date('Y-m-d H:i', strtotime("+{$agenda->getDiferenca()} weeks", strtotime($tempDataAgenda)));
                            $dataAgenda = date('Y-m-d', strtotime($tempDataAgenda));
                            $horario = date('H:i', strtotime($tempDataAgenda));
...
//                $sql = "SELECT * FROM usuario where (email = :email OR nome = :email) AND (senha = :senha)";
        ...
        function concluirCOmpromisso(Agenda $agenda){
            $db = new conexao();

            try{
                
                $sql = "UPDATE agenda SET status = 1 where idAgenda = :idAgenda";
                
                
                $stmt = $db->getConn()->prepare($sql);

                $stmt->execute(array(':idAgenda' => $agenda->getIdAgenda()));

                echo json_encode(array("response" => $stmt->rowCount() ), JSON_UNESCAPED_UNICODE);
        ...
        function excluirCompromisso(Agenda $agenda){
            $db = new conexao();
